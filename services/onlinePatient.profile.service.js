...
module.exports = {    
    ...
    UpdateOneOnlinePatientProfile : (req, res) => {
        onlinePatientProfile.findOneAndUpdate(req.params.email, req.body, { new: true })
            .then((oPatient) => {
                if (!oPatient) {
                    return res.status(404).send({
    ...
    DeleteOneOnlinePatientProfile : (req, res) => {
        onlinePatientProfile.findByIdAndDelete(req.params.id)
            .then(oPatient => {
                if(!oPatient) {
                return res.status(404).send({
                message: "Online Patient profile not found with email " + req.params.id
                });
                }
